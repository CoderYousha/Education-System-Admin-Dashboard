...
function AdsDetails({ onClickClose, onClickUpdate, banner, setSnackBar, setBanners }) {
    ...
    return (
        <>
            {
                banner &&
                <Box sx={{ backgroundColor: theme.palette.background.paper }} className="shadow-lg w-3/5 h-fit rounded-3xl px-4 py-5 overflow-y-scroll none-view-scroll max-sm:w-4/5 max-sm:translate-x-0 max-sm:left-0 relative max-sm:h-screen" dir={language === 'en' ? 'ltr' : "rtl"}>
